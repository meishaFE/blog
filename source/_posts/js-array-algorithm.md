...
基本思路如下：
在循环这个 9*9 的二维数组时，假设循环的行数组下标为 arr[i][j]，那么列数组可以通过 arr[j][i]这种规律来判断。
同时，3*3 的数组也可以通过找到某种特定的与横向数组对应的规律来组合判断。
...
这几个题目当中的解题思路个人觉得是比较典型的，充分利用了map / set等数据结构来解决，希望这些问题可以为之后开发过程中遇到的新问题有一点解决灵感～～
